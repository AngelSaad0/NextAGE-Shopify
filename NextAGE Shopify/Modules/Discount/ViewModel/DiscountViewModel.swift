...
class DiscountViewModel {
    ...
    private func calcSubTotal() {
        ...
        bindSubtotalPrice(String(format: "%.2f", subTotal) + " \(UserDefaultManager.shared.currency)")
    }
    
    private func calcTotal() {
        calcSubTotal()
        bindDiscountPrice(String(format: "%.2f", discountAmount) + " \(UserDefaultManager.shared.currency)")
        bindTotalPrice(String(format: "%.2f", subTotal + (Double(discountAmount))) + " \(UserDefaultManager.shared.currency)")
    }
}
